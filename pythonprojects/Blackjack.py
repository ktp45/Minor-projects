...
def make_deck(num, temp_deck=None): # deck generating function
    if temp_deck is None:
       temp_deck = []
    for x in range (num) : #if the number stored in num,which is the total cards, is dividable by 52, that would make num/52 decks in one
...
def print_statement(dealer_hand=None): #interface statement print
    if dealer_hand[0].get_name()=='A':
        return "Press 1-Hit ; 2-Stand ; 3-Double ; 4-Insurance:\n" #if the dealer has ace players can bet on insurance 2 to 1
    else:
        return "Press 1-Hit ; 2-Stand ; 3-Double ; 4-Surrender:\n" #otherwise they can surrender and get half of their money

def war(dealer_hand=None,player_hand=None): #Blackjack war func
    print_hand(dealer_hand)
    print_hand(player_hand)
    if calc_score(player_hand)>calc_score(dealer_hand): #the war is won only if the first player card is bigger than the dealer
...
def win_definer(player_hand=None,dealer_hand=None):
    if calc_score(dealer_hand)>21 or (calc_score(dealer_hand) < calc_score(player_hand) and calc_score(dealer_hand)<21):
...
def cheat(deck=None): # function showing the deck if the cheat bet is activated
    ...
    for j in range(len(deck)):
        print(deck[j], end=' ')
        if j % 31 == 30 or j == len(deck) - 1: print()
...
def main_loop(number_decks=0, money=0): # main operating function
    ...
    while money >= 0 and len(deck) > 0:
        ...
        if bet != 0 and bet <= money:
            ...
            while not winner:
                if len(deck)<=38: # if the player has 21 aces and the dealer 17 there will be 38 cards which is the maximum in 1 game
                ...
                if calc_score(player_hand)==21 and dealer_hand[0].get_bjpower != (1 or 10):
                    print("Blackjack! You win!") #if the player has 21 and dealer has no chance of having 21 in 2 cards there is a Blackjack
                    money+=bet*2.5
                    break

                decision = player_decision(dealer_hand,player_hand,deck) #the house wins because the player has a choice
                if decision[1]:
                    money-=bet;bet*=2
                    print(bet)
                if decision[0] and dealer_hand[0].get_name()=='A':
                    ins_bet=bet;money-=bet
