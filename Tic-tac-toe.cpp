	...
	*/
	...
	if(mode==2)
	{
		...
		while(winner!=1)
		{
			...
			if(br%2==0)board[x-1][y-1]=1; //fill board with X or O
	        ...
	        for(int i=0;i<size;i++)
               	{
		            for(int j=0;j<size;j++)
		             ...
		             else if(board[i][j]==2)cout<<"["<<"O"<<"]"; // draw updated board
		             else cout<<"[ ]";
		              }
		                 cout<<endl;
	            }
		for(int i=0;i<size;i++)
			{
				for(int j=0;j<size;j++)
				{
					if(board[i][j]==1 && board[i][j+1]==1)br2++;
					if(board[j][i]==1 && board[j+1][i]==1)br3++; // check for winner by rows and cols
					if(br2==size-1 && !winner)
					{
					if(board[i][j]==1)cout<<"PLayer 1 won!"<<endl;
					else cout<<"CPU won!"<<endl;
						winner=1;
					}
						if(br3==size-1 && !winner)
					{
					if(board[j][i]==1)cout<<"PLayer 1 won!"<<endl;
					else cout<<"CPU won!"<<endl;
						winner=1;
					}
				}
			...
			for(int i=0;i<size;i++)
			{
				if(i==size-1 && !winner)
				{
						if(board[i][i]==1)cout<<"PLayer 1 won!"<<endl; // check for winner by main diag
					else cout<<"CPU won!"<<endl;
					winner=1;
				}
	...
	else if(mode==1)
	{
		...
		while(winner!=1)
		{
			...
			for(int i=0;i<size;i++)
			{
				...
				{
					if(board[i][j]==1 && board[i][j+1]==1)br2++;
					if(board[j][i]==1 && board[j+1][i]==1)br3++;
					if(br2==size-1 && !winner)
					{
					if(board[i][j]==1)cout<<"PLayer 1 won!"<<endl;
					else cout<<"Player 2 won!"<<endl;
						winner=1;
					}
						if(br3==size-1 && !winner)
					{
			...
			for(int i=0;i<size;i++)
			{
				if(i==size-1 && !winner)
				{
						if(board[i][i]==1)cout<<"PLayer 1 won!"<<endl;
					else cout<<"Player 2 won!"<<endl;
					winner=1;
				}
				if(board[i][i]!=board[i+1][i+1])break;
